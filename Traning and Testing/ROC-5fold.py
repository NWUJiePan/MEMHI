...
while counter0 < 5:
    ...
    RAPName = 'EF_RealAndPredictionA+B' + str(counter0) + '.csv'
    RAPNameProb = 'EF_RealAndPredictionProbA+B' + str(counter0) + '.csv'
...
while counter < len(mean_fpr):
    ...
    counter = counter + 1
storFile(FprAndTpr, 'EF_RNAInterFprAndTprA+B.csv')
...
plt.title('Receiver operating characteristic curves: AUC={0:0.4f}'.format(mean_auc))
# 画网格
plt.grid(linestyle='--')
...
# # 保存图片
plt.savefig('EF_ROC-5fold.tif', dpi=500)
plt.show()
